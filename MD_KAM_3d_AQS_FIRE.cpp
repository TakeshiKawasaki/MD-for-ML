...
#define teq 1000 //equilibration time
...
void calc_force(double (*x)[dim],double (*f)[dim],int *a,double *U,int (*list)[Nn],double gamma){
  double dx,dy,dz,dr2,dUr,w2,w6,w12,w2cut,w6cut,w12cut,aij,eij,dUrcut,Ucut,dr;
  ini_array(f);
  *U=0;
  for(int i=0;i<Np;i++)
    for(int j=1;j<=list[i][0];j++){
...
int Fire(double (*x)[dim],double (*f)[dim],double gamma,int (*list)[Nn],int *a,int M,double *U)
{
  ...
  double alpha = 0.1,P;
  double dt0=0.00001;
  double v[Np][dim],x0[Np][dim],dx,dy,dz,dxmax,dymax,dzmax;
  ...
  for(;;){
	...
	x[i][j] += v[i][j]*dt0;
      ...
      for(j=0;j<dim;j++)
	v[i][j] = (1.0-alpha)*v[i][j]+alpha*f[i][j]/sqrt(f[i][0]*f[i][0]+f[i][1]*f[i][1]+f[i][2]*f[i][2]+DBL_EPSILON)*sqrt(v[i][0]*v[i][0]+v[i][1]*v[i][1]+v[i][2]*v[i][2]);
    ...
    //    printf("sum_force=%.16f,dt=%f,x=%f, alpha=%f,P=%f,gamma=%f \n",sum_force,dt0,x[0][0],alpha,P,gamma);
    if(P>=0){
      conv++;
      if(conv>5){
	dt0*=finc;
...
int main(){
  double x[Np][dim],x_update[Np][dim],v[Np][dim],f[Np][dim],kine;
  int list[Np][Nn],a[Np];
